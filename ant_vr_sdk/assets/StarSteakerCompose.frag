...
void main()
{
	vec2 TexCoord = ScreenSpaceUV.xy;
	vec4 color1 = max(texture(g_Texture1, TexCoord), texture(g_Texture2, TexCoord));
	vec4 color2 = max(texture(g_Texture3, TexCoord), texture(g_Texture4, TexCoord));
	f4FragColor = max(color1, color2);
}
